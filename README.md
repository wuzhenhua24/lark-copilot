...
#### 平台权限申请 ≠ 用户 OAuth 授权（为什么 step 5 不能跳过）

常见误解："我在平台已经勾了用户身份 scope 并发版审批了，那 `lark-cli auth login` 是不是
就不用跑了？" —— 不行，这是两层不同的事：

| 步骤 | 在哪做 | 做什么 | 产物 |
|---|---|---|---|
| 平台权限申请 | 飞书开放平台 → 权限管理（管理员） | 声明应用**有权申请**哪些 scope | scope 白名单（应用层、一次性） |
| `lark-cli auth login` | 部署机 / 浏览器 OAuth（每个使用者） | 使用者**授权这个应用读写自己的账号** | `user_access_token`（per-user、per-机器） |

类比餐厅：

- 平台权限申请 = 餐厅备案「我家菜单上允许卖哪些菜」
- `lark-cli auth login` = 客人到店点单、签收并把菜端走

为什么不能跳过 login：

1. 平台审批**不会发放 token**。没 token，lark-cli 调任何 API 都是 401
2. user token 必须**绑定到具体用户** —— 管理员替你审批 scope ≠ 替你授权使用，
   每个跑 router 的人都要自己 OAuth 一次，token 才属于"他自己"
3. token 存在**部署机本地**（lark-cli config）。换机器、清配置后要重 login
4. OAuth 浏览器里的"同意授权"是**用户本人的授权动作**，平台审批替代不了

`lark-cli auth login --scope xxx` 里的 `--scope` 是"**从平台菜单里挑这次要点的 scope**"，
不是申请新权限：
...
三步缺一不可。平台没审批的 scope，login 那步会失败或被悄悄丢弃。

实操对应：

| 时机 | 谁做 | 做什么 |
|---|---|---|
| 应用初始化 | 管理员（一次） | 平台勾 scope + 发版 + 审批 |
| 每个新部署人 | 部署者本人（每人一次） | `lark-cli auth login --scope ...`，浏览器同意 |
| 换机器 / token 失效 | 部署者本人 | 再 login 一次 |
| 加了新 scope | 管理员发新版 + 所有人重新 login | 旧 token 里没有新 scope |
...
#### OAuth 自动补全权限申请（为什么 login 后两列都被打勾）

在部署机跑 `lark-cli auth login --scope ...` 后，跳转到管理员审批，回去看权限管理页面，
常会发现**应用身份列和用户身份列同时被勾上一批 scope**，远超你在命令行写的那几个。
这是正常行为，不是越权。

**机制**：

1. `--scope` 里如果有应用还没声明过的 scope，OAuth **不会直接报错**
2. 平台用一个"**自动补全权限申请**"流程：把这些 scope 加进权限管理、打包推给管理员
3. 这个自动加的过程，把**同名 scope 的所有身份列都打勾**（不只是 OAuth 实际要用的那一列）
4. 叠加之前订阅 `im.message.receive_v1` 时强制拉进来的那批 app-identity scope，
   最终列表就显得"两边都很多"

**为什么平台要这么做**：scope 模型里"身份列"只是同一权限对象的两种投射，
自动补全不知道你将来会不会切 tenant token 用，索性两列一起申请，省一次审批往返。
是开发体验优化，不是安全策略。
...
| 维度 | 状况 |
|---|---|
| 运行时 | 0 影响。代码全程 `--as user`，从不用 tenant token，app-identity scope 没有 code path 走 |
| 安全面 | 等价于"菜单上多印了几道菜，但厨房从来不做"。真正的风险点是 `app_secret` 泄露 |
| 合规 | 若公司有最小权限审计，需解释这是平台 OAuth 自动补的，不是项目主动申请 |

**要不要手动清理 app-identity 那列**：

- **不建议清理**。里面有些是事件订阅强制依赖的（`im:message.group_at_msg:readonly` 等），
  删了 `im.message.receive_v1` 订阅会失效
- **如要清**：只清和 user scope 对称的"镜像项"，保留事件订阅依赖的几个；
  清完要**重新发版审批**，并且**所有人重新 login**（不然旧 token 还是旧 scope 集）
- 判断方法：去「事件与回调」展开 `im.message.receive_v1` 的"所需权限"列表 ——
  那些是依赖项必须保留，剩下的就是镜像项

实操建议：**留着不管，把 `app_secret` 保管好**才是重点。
